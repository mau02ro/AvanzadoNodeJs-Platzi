...
class PlatziverseAgent extends EventEmitter {
  constructor(opst) {
    super(); // Llamar al constructor de la clase que estamos extendiendo
    this._options = defaults(opst, options);
  ...
  connect() {
    if (!this._started) {
      this._client = mqtt.connect(this._options.host);
      ...
      this._client.on('agent/message', (topic, payload) => {
        ...
        switch (topic) {
          ...
          case 'agent/message':
            broadcast =
              payload && payload.agent && payload.uuid !== this._agentId;
            break;
        }
